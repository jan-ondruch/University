...
*/
	...
	/* zacatek + prvni volne misto na 0*/
	q->f_index = 0;	
	q->b_index = 0;

	/* hodnoty prvku pole nastavit na * */
...
*/
	...
	if(queueEmpty(q)) {
		queueError(QERR_FRONT);	// je prazdna
		return;
	}

	*c = q->arr[q->f_index];	// c se zacatku fronty
...
*/
	...
	if(queueEmpty(q)) {
		queueError(QERR_REMOVE);	// je prazdna
		return;
	}
...
*/
	...
	if(queueEmpty(q)) {
		queueError(QERR_GET);	// je prazdna
		return;
	}

	queueFront(q, c);	// vrat znak ze zacatku fronty
	queueRemove(q);	// odstran znak
