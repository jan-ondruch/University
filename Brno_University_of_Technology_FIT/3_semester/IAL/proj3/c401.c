...
**/
	...
	// set root to null => initialization
	*RootPtr = NULL;
...
**/
	...
	if (!(*RootPtr)) {
		...
		tBSTNodePtr node;	// new node for data

		node = malloc(sizeof(struct tBSTNode));
		if (!node) {
			return;
		}
		...
		*RootPtr = node;
		...
		// left subtree -> save left
		if (K < (*RootPtr)->Key) {
			BSTInsert(&((*RootPtr)->LPtr), K, Content);
		}
		else {

			// right subtree -> save right
			if (K > (*RootPtr)->Key) {
				BSTInsert(&((*RootPtr)->RPtr), K, Content);
			}
			// found the place -> save new data
			else {
...
**/
	...
	// right node exists -> go right
	if ((*RootPtr)->RPtr) {
		ReplaceByRightmost(PtrReplaced, &(*RootPtr)->RPtr);
	}
	// right does not exist -> save data
	else {

		PtrReplaced->Key = (*RootPtr)->Key;
		PtrReplaced->BSTNodeCont = (*RootPtr)->BSTNodeCont;

		// save the tree to temp & create a new node 
		temp = *RootPtr;	
...
**/
	...
	if ((*RootPtr)) {
		...
		// search has not ended, the node might be still in the tree -> go left
		if (K < ((*RootPtr)->Key)) {
			BSTDelete(&((*RootPtr)->LPtr), K);
		}
		else {

			// K is bigger -> continue right
			if (K > ((*RootPtr)->Key)) {
				BSTDelete(&((*RootPtr)->RPtr), K);
			}
			else {			
				// RootPtr will be deleted
				temp = (*RootPtr);	// save the root
...
**/
	...
	// values for saving left / right heights
	int valL, valR;
	...
	if (NodePtr) {
		...
		// call the fuction for both left and right branch
		valL = BSTHeight(NodePtr->LPtr, TRUE);
		valR = BSTHeight(NodePtr->RPtr, TRUE);
...
**/
	...
	if ((*RootPtr)) {
		...
		// firstly we go left, then right & free the nodes along the way
